 ...
 *
 * About this file:
 * This is a derivative of the main kiezatlas.js client working as a js-utility in this `kiezatlas-page` plugin.
 */
...
var kiezatlas = new function() {
    ...
    this.searchRequest = function(query, handler) {
        if (typeof query == "undefined") return null
        else queryString = query
        // if(debug) log('start searching for ' + queryString);
        queryString = encodeURIComponent(query) // new String(queryString).replace("/ /g", "%20")
        // var url = 'http://www.kiezatlas.de/rpc/'
        var url = kiezatlas.serviceUrl + '/proxies/searchGeoObjects.php?query=' + queryString
        jQuery.ajax({
          type: "GET",
          url: url,
          ...
          success: function(obj){
                kiezatlas.searchResults = obj.result
                console.log(kiezatlas.searchResults)
                if (handler !== undefined) handler(query)
          },
          error: function(x, s, e){
                console.log(e)
                console.log('Fehler beim Suchen aufgetreten: ' + x.statusText);
                if (handler !== undefined) handler("zero")
          }
        });
    }
    ...
    this.loadCityMapTopics = function (mapId, workspaceId, handler) {
        var url = kiezatlas.serviceUrl + '/proxies/getCityMap.php?mapId='+mapId+'&workspaceId='+workspaceId;
        ...
        jQuery.ajax({
            ...
            success: function(obj) {
                console.log("Loaded City-Map-Topics: ")
                console.log(obj)
    ...
    this.loadWorkspaceCriterias = function (mapId, handler) {
        // var url = 'http://www.kiezatlas.de/rpc/'
        var url = kiezatlas.serviceUrl + '/proxies/getWorkspaceCriterias.php?mapId='+mapId
        // var body = '{"method": "getMapTopics", "params": ["' + mapId+ '"]}'
        jQuery.ajax({
            type: "GET", async: false, url: url, dataType: 'json',
                beforeSend: function(xhr) { 
                    xhr.setRequestHeader("Content-Type", "application/json")
                    // xhr.setRequestHeader("Origin", "http://www.kiezatlas.de")
            },
            success: function(obj) {
  ...
  this.getMapTopics = function(topics) {
  ...
  this.makeEmailLink = function (url, label) {
    ...
    this.getAllTopicsInCat = function (catId) {
    ...
    this.getCategory = function(categoryId) {
        ...
        for (var i = 0; i < kiezatlas.workspaceCriterias.result.length; i++) {
            var criteria = kiezatlas.workspaceCriterias.result[i]
            for (var k = 0; k < criteria.categories.length; k++) {
                // looping over all cats of a crit
                var id = criteria.categories[k].catId
                if (id == categoryId) {
                    category.icon = criteria.categories[k].catIcon;
                    category.name = criteria.categories[k].catName;
                    return category;
                }
            }
        }
